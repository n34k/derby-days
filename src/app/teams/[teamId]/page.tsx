...
export default async function TeamPage({ params }: {params: Promise<{ teamId: string }>}) {
  ...
  const derbyDarlingInfo = team.derbyDarlingName || team.derbyDarlingImageUrl ? { name: team.derbyDarlingName ?? null, image: team.derbyDarlingImageUrl ?? undefined} : null;

  let headCoachInfo: { name: string | null; image?: string | null } | null = null;

  if (team.headCoachId) {
    const headCoach = await prisma.user.findUnique({ where: { id: team.headCoachId }});
    if (headCoach) {
      headCoachInfo = {name: headCoach.name ?? null, image: headCoach.image ?? undefined,};
    }
  }

  const coaches = await prisma.user.findMany({ where: { teamId: team.id, globalRole: "BROTHER" }});
  const coachesInfo = coaches.map((coach) => ({ name: coach.name ?? null, image: coach.image ?? undefined}));
