...
const UpdateUserForm = ({ initialName, initialImage, initialWalkoutSong, initialPublicId }:Props) => {
  ...
  const submitUpdate = async (updatedName = name, updatedImage = image, updatedWalkoutSong = walkoutSong, updatedPublicId = imagePublicId) => { 
  ...
  const handleSubmit = async (e: React.FormEvent) => { //for form
    e.preventDefault();
    try {
      const res = await fetch("/api/user", {
        method: "PATCH",
