...
const AdPurchaseTable: React.FC<Props> = ({ ads, users, teams }) => {
    ...
    const handleUpload = async (info: CloudinaryUploadWidgetInfo, adId: string) => {
        setUploadingId(adId);
        const res = await fetch(`/api/admin/adPurchase/${adId}`, {
            method: "PATCH",
                    ...
                    <div className="overflow-x-auto w-full">
                        <table className="w-full border border-base-content text-sm">
                            ...
                            <tbody>
                                {ads.map((ad) => {
                                    const userName =
                                        ad.userId && userNameById.get(ad.userId) ? userNameById.get(ad.userId) : null;
                                    ...
                                    return (
                                        <tr key={ad.id}>
                                            ...
                                            <td className="border px-2 py-1 text-center">
                                                {ad.teamId ? greekLetters(ad.teamId) : "—"}
                                            </td>

                                            {/* User */}
                                            <td className="border px-2 py-1 text-center">{userName ?? "—"}</td>
...
export default AdPurchaseTable;
