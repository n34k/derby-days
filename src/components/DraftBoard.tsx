            ...
            <table className="min-w-full border-collapse text-sm">
                <thead className="bg-base-300">
                    <tr>
                        ...
                        {teams.map((t) => (
                            <th key={t.id} className="text-2xl p-3 text-center">
                                {greekLetters(t.id)}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody>
                    {Array.from({ length: rounds }).map((_, rIdx) => (
                        <tr key={rIdx}>
                            <td className="p-3 border-1 border-secondary font-semibold text-center text-lg">
                                {rIdx + 1}
                            </td>
                            {teams.map((t, cIdx) => {
                                ...
                                return (
                                    <td
                                        key={t.id}
                                        className="border-1 border-secondary text-center"
                                    >
                                        {cell?.name ?? (
