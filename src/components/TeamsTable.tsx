  ...
  const handleUploadDerby = async (info: CloudinaryUploadWidgetInfo, teamId: string) => {
    try {
      ...
      setTeamState((prev) =>
        prev.map((t) =>
          t.id === teamId
            ? freshTeam ?? { ...t, derbyDarlingImageUrl: body.derbyDarlingImageUrl, derbyDarlingPublicId: body.derbyDarlingPublicId }
            : t
        )
      );
                      ...
                      <td className="border px-2 py-1">
                        {editing ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(team.id, "tshirtsSold", parseInt(e.target.value || "0", 10))
                            }
                          />
                        ) : (
                          team.tshirtsSold ?? "—"
                      ...
                      <td className="border px-2 py-1">
                        {editing ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(team.id, "moneyRaised", parseFloat(e.target.value || "0"))
                            }
                          />
                        ) : (
                      ...
                      <td className="border px-2 py-1">
                        {editing ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(team.id, "points", parseInt(e.target.value || "0", 10))
                            }
                          />
                        ) : (
                          team.points ?? "—"
